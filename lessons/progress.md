...
- 2026-07-13: IMPLEMENTED M2 `/extract` (structured output or typed error; forces #3/#5/#7). New `services/chat/src/extract.py`: `extract()` runs one of two modes chosen per request — `native` (sends a `response_format` json_schema; the proxy translates it per provider — Anthropic→forced tool call, OpenAI→strict json_schema; `drop_params:true` can silently drop it) vs `prompt` (schema embedded in the prompt, no enforcement). Both modes exist to feed Break-It #2's native-vs-prompt failure-rate comparison. Parsed output is validated with `jsonschema` regardless of mode; a parse/validation error is fed back into the messages and re-prompted up to `EXTRACT_MAX_RETRIES` (default 2 → 3 attempts), then a typed `ExtractionError` → endpoint returns a clean `422` `ExtractError` (kind parse/schema/bad_schema/upstream), never malformed output, never a 500. `temperature=0`, non-streaming, `X-Trace-Id` on every response, each attempt a separate generation under one trace (+ `extract_attempts`/`extract_outcome` scores); malformed input schema rejected up front with no model call; broad provider errors caught → typed 502. First tests in the repo (`services/chat/tests/test_extract.py`, httpx ASGITransport + mocked `litellm.completion`): happy path, retry-fires-once, exhausted→typed-error, bad-schema-no-call, native-vs-prompt enforcement — 5 pass; ruff/format/basedpyright(0/0/0) clean; app boots and the bad-schema 422 verified over real HTTP. STILL THE LEARNER'S TO OWN (deliberately not written for them): the mechanism-level *why* for temp=0, the *justification* for N, and the one-sentence *why-no-streaming* — code carries starter comments only. PENDING teacher gate + live verification: the happy/retry/typed-error paths against the real proxy and pointing at the retry loop firing in a LangFuse trace (acceptance criterion line 26 left unchecked; that's evidence the teacher checks, and it doubles as Break-It #2).
